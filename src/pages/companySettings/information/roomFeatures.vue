<template>
    <Breadcrumbs title="Oda Özellikleri" main="İşletme Ayarları"  />
   <div>
     <!-- ADD MODAL   -->
    <div class="col-12">
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="add_departmentLabel">Yeni Oda Özelliği</h5>
                        ...
                        <form class="form-bookmark needs-validation" id="bookmark-form" novalidate="">
                            <div class="row">
                                <div class="mb-3 col-md-12">
                                    <label for="task-title">Özellik Adı</label>
                                    <input class="form-control" id="task-title" type="text" v-model="editRoomFeaturesObj.Title" required=""
                                        autocomplete="off">
                                </div>
        ...
        <div class="modal fade modal-bookmark" id="edit_department" tabindex="-1" role="dialog"
            ...
            <div class="modal-dialog modal-md" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="edit_departmentLabel">Oda Özelliği</h5>
                        <button class="btn-close" type="button" data-bs-dismiss="modal" aria-label="Close"> </button>
                    </div>
                    <div class="modal-body">
                        <form class="form-bookmark needs-validation" id="bookmark-form" novalidate="" @submit.prevent="editButton">
                            <div class="row">
                                <div class="mb-3 col-md-12">
                                    <label for="task-title">Özellik Adı</label>
                                    <input class="form-control" id="task-title" type="text" v-model="editRoomFeatures.title" required=""
                                        autocomplete="off">
                                </div>
                                <div class="media">
                        ...
                        <div class="media-body text-end icon-state switch-outline">
                          <label class="switch">
                            <input type="checkbox" checked v-model="editRoomFeatures.isActive"><span class="switch-state bg-success"></span>
                          </label>
                        </div>
                      </div>
        ...
        <table class="table">
          <thead>
            ...
            <tr class="border-bottom-primary">
              <th scope="col">No</th>
              <th scope="col">Oda Özellik Adı</th>
          ...
          <tbody>
            <tr v-for="(item,index) in GetRoomFeatures" :key="item.id" class="border-bottom-primary">
...
export default {
  ...
  methods:{
    ...
    saveButton(){
     ...
     this.editRoomFeaturesObj.userID=  this.userID
     this.editRoomFeaturesObj.ModifiedUserName =this.userID
      this.AddData({ db: "RoomFeatures", dataSet: this.editRoomFeaturesObj }).then(p => {
        $(".btn-close").click();
        this.myQuery({ tableName: "RoomFeatures", updateOpt: "SetRoomFeatures" })
        
       this.editRoomFeaturesObj={}
    })
  },
  editSendData(value){
    this.editRoomFeatures=value;

 
  },
    editButton(e){
      const uid = JSON.parse(localStorage.getItem("storedData")).userUid;
      this.editRoomFeatures.ModifiedOn=currentDate,
     
      this.editRoomFeatures.ModifiedUserName=uid
      
      this.EditData({ db: "RoomFeatures", data: this.editRoomFeatures, id: this.editRoomFeatures.id }).then(t => {
        $(".btn-close").click();
        this.editRoomFeaturesObj={}
 ...
 console.log(data)
  ...
  dateFormat(value) {
    ...
    deleteButton(delData){
      this.$swal({
          ...
          if(result.value){
            this.DeleteData({ db: "RoomFeatures", mt: "DeleteDataSet", data: delData }).then(p => {
  ...
  watch:{
    editRoomFeatures(value){
      this.editRoomFeaturesObj=value
    }
  }
