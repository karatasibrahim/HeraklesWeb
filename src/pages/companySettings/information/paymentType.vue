<template>
  <Breadcrumbs title="Ödeme Tipi Tanımlama" main="İşletme Ayarları"  />
 <div>
   <!-- ADD MODAL   -->
  <div class="col-12">
      <div class="modal fade modal-bookmark" id="add_paymentType" tabindex="-1" role="dialog"
          aria-labelledby="add_paymentTypeLabel" aria-hidden="true">
          <div class="modal-dialog modal-lg" role="document">
              <div class="modal-content">
                  <div class="modal-header">
                      <h5 class="modal-title" id="add_paymentTypeLabel">Yeni Ödeme Tipi</h5>
                      ...
                      <form class="form-bookmark needs-validation" id="bookmark-form" novalidate="">
                          <div class="row">
                              <div class="mb-3 col-md-12">
                                  <label for="task-title">Ödeme Tipi Adı</label>
                                  <input class="form-control" id="task-title" type="text" v-model="paymentTypeObj.title" required=""
                                      autocomplete="off">
                              </div>
       ...
       <!-- EDİT MODAL  -->
      <div class="modal fade modal-bookmark" id="edit_paymentType" tabindex="-1" role="dialog"
          aria-labelledby="edit_paymentTypeLabel" aria-hidden="true">
          <div class="modal-dialog modal-lg" role="document">
              <div class="modal-content">
                  <div class="modal-header">
                      <h5 class="modal-title" id="edit_paymentTypeLabel">Yeni Ödeme Tipi</h5>
                      <button class="btn-close" type="button" data-bs-dismiss="modal" aria-label="Close"> </button>
                  </div>
                  <div class="modal-body">
                      <form class="form-bookmark needs-validation" id="bookmark-form" novalidate="" @submit.prevent="editButton">
                          <div class="row">
                              <div class="mb-3 col-md-12">
                                  <label for="task-title">Ödeme Tipi Adı</label>
                                  <input class="form-control" id="task-title" type="text" v-model="editPaymentType.title" required=""
                                      autocomplete="off">
                              </div>
                              <div class="media">
                      ...
                      <div class="media-body text-end icon-state switch-outline">
                        <label class="switch">
                          <input type="checkbox" checked v-model="editPaymentType.isActive"><span class="switch-state bg-success"></span>
                        </label>
                      </div>
                    </div>
  ...
  <div class="card">
    ...
    <div class="table-responsive">
      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="(item,index) in GetPaymentType" :key="item.id" class="border-bottom-primary">
          ...
          <div class="dropdown-menu dropdown-menu-right  ">
            <a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#edit_paymentType"
...
methods:{
  ...
  saveButton(){
   ...
   this.paymentTypeObj.userID=  this.userID
   this.paymentTypeObj.ModifiedUserName =this.userID
    this.AddData({ db: "PaymentType", dataSet: this.paymentTypeObj }).then(p => {
      $(".btn-close").click();
      this.myQuery({ tableName: "PaymentType", updateOpt: "SetPaymentType"})
      
     this.paymentTypeObj={}
  })
},
editSendData(value){
  this.editPaymentType=value;


},
  editButton(e){
    const uid = JSON.parse(localStorage.getItem("storedData")).userUid;
    this.editPaymentType.ModifiedOn=currentDate,
   
    this.editPaymentType.ModifiedUserName=uid
    
    this.EditData({ db: "PaymentType", data: this.editPaymentType, id: this.editPaymentType.id }).then(t => {
      $(".btn-close").click();
      this.paymentTypeObj={}
...
dateFormat(value) {
  ...
  deleteButton(delData){
    this.$swal({
        ...
        if(result.value){
          this.DeleteData({ db: "PaymentType", mt: "DeleteDataSet", data: delData }).then(p => {
