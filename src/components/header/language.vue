        ...
        <div class="translate_wrapper " :class="{ 'active': isActive }">
            <div class="current_lang">
                <div class="lang"><i class="flag-icon" :class="this.langIcon" @click="display()"></i><span
                        class="lang-txt">{{ this.langLangauge }}</span></div>
            </div>
...
export default {
    ...
    mounted() {
        this.$store.dispatch('setLang', { name: localStorage.getItem('currentLanguage') || 'TR', icon: localStorage.getItem('currentLanguageIcon') || 'flag-icon-tr' })
    }
}
</script>
